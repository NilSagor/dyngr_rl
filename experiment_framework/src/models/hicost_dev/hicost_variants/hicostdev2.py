...
def build_cooccurrence_graph(edge_index, num_nodes):
    """
    edge_index: [2, E] (undirected, each edge appears once)
    returns edge_index (2, E_cooc) and edge_weight (E_cooc)
    """
    ...
    for u in range(num_nodes):
        ...
        for v in range(u+1, num_nodes):
            if not adj_list[v]:
                continue
            inter = len(adj_list[u] & adj_list[v])
            if inter == 0:
                continue
            uni = len(adj_list[u] | adj_list[v])
            jaccard = inter / uni
            rows.append(u); cols.append(v); weights.append(jaccard)
            rows.append(v); cols.append(u); weights.append(jaccard)

    edge_index_cooc = torch.tensor([rows, cols], dtype=torch.long, device=edge_index.device if torch.is_tensor(edge_index) else None)
    edge_weight_cooc = torch.tensor(weights, dtype=torch.float, device=edge_index.device if torch.is_tensor(edge_index) else None)
    return edge_index_cooc, edge_weight_cooc
...
class HiCoSTdev2(L.LightningModule):
    def __init__(self, config:HiCoSTdev2Config):
        ...
        if self.enable_walk:
            ...
            self.walk_sampler = MultiScaleWalkSampler(
                ...
                walk_length_short=config.walk_length_short,
                walk_length_long=config.walk_length_long,
                walk_length_tawr=config.walk_length_tawr,
                num_walks_short=config.num_walks_short,
                num_walks_long=config.num_walks_long,
                num_walks_tawr=config.num_walks_tawr,
                temperature=config.walk_temperature,
                memory_dim=config.memory_dim,
                time_dim=config.time_dim,
                time_noise_std=config.walk_time_noise_std,
                mask_prob=config.walk_mask_prob,
                adaptive_length_factor=config.walk_adaptive_factor,
                use_temporal_bias=config.use_temporal_bias,   # False for A2
            )
            ...
            if self.use_time_delta_attention:
                self.walk_encoder = TimeDeltaAttentionWalkEncoder(
        ...
        self.use_explicit_co_gnn = getattr(config, 'use_explicit_co_gnn', False)
        self.co_gnn_out_dim = 32  # default, will be overwritten if co_gnn is created
        if self.use_explicit_co_gnn:
            # Co-GNN dimensions: input = final_emb_dim (before adding co-gnn output)
            # We will later concatenate c_u, so we increase final_emb_dim by out_dim
            self.co_gnn = CoGNN(
            ...
            # Increase final embedding dimension by 32
            self.final_emb_dim += self.co_gnn_out_dim
            # Recreate affinity_score with new dimension
            self.affinity_score = AffinityMergeLayer(
    ...
    #     """Called once when the trainer begins. Initialise Co-GNN embeddings."""
    #     if self.use_explicit_co_gnn:
    #         if hasattr(self.neighbor_finder, 'edge_index') and self.neighbor_finder.edge_index is not None:
    #             edge_index = self.neighbor_finder.edge_index.to(self._device)
    ...
    #             logger.info("Co-GNN embeddings initialised on fit start")
    #         else:
    #             logger.warning("No edge_index in neighbor_finder; Co-GNN will be disabled")
    ...
    def compute_temporal_embeddings(self, source_nodes, destination_nodes, negative_sources, negative_destinations,
        ...
        walk_data = self.walk_sampler(
            ...
            current_times=torch.tensor(edge_times, device=self._device, dtype=torch.float32),
            memory_states=memory if memory is not None else self.node_raw_features,
    ...
    def compute_walk_embeddings(self, *args, **kwargs):
        # kept only for fallback when use_time_delta_attention=False
        pass
    ...
    #     :return:
    #     """
    #     # Get walk length from the shape
    #     walk_len = nodes_neighbor_ids.shape[2]   # shape: (batch, num_walks, walk_len)
    #     device = self._device   # or use neighbor_raw_features.device
    ...
    #     if self.use_time_delta_attention:
    #         # Prepare features: concatenate raw + time + edge + position
    #         # shape: [batch, num_walks, walk_len, input_dim]
    #         combined_features = torch.cat([neighbor_raw_features, neighbor_time_features, edge_features, neighbor_position_features], dim=-1)
            
    #         anon_ids = walk_data[side][walk_type]['nodes_anon']  # shape [B, num_walks, walk_len]
            
    #         anon_embeds = self.walk_sampler.anon_embedding(anon_ids)  # shape [B, num_walks, walk_len, hidden_dim]
            
    #         mask = (torch.arange(walk_len, device=device).unsqueeze(0).unsqueeze(0) < torch.tensor(walks_valid_lengths, device=device).unsqueeze(-1)).float()
