...
class TGNv8(BaseEnhancedTGN):
    ...
    4. ONLY use HCT walk embeddings + SAM memory (remove Base TGN graph attention/messaging)
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.use_sam:
            ...
            logger.info(f"TGNv8 initialized: {sam_info}WalkSampler + "
        ...
        # Fusion layer (modified: only single input now)
        self.fusion_layer = nn.Sequential(
            nn.Linear(hidden_dim, hidden_dim),  # 移除 *2，因为不再融合base TGN特征
            nn.LayerNorm(hidden_dim, eps=1e-4),
            nn.ReLU(),
            nn.Dropout(dropout)
        )
        ...
        logger.info(f"TGNv8 initialized (Modified - No Base TGN): SAM({num_prototypes}p) + WalkSampler + "
                   f"{'HCT(' + str(hct_d_model) + 'd, ' + str(hct_nhead) + 'h)' if use_hct else 'SimpleWalkEncoder'}")
    ...
    def compute_temporal_embeddings(
        ...
        """
        Modified: Unified embedding computation WITHOUT Base TGN graph attention/messaging
        Only uses SAM memory + walk embeddings (HCT/simple)
        """
        device = self.device
        
        # ----- 1. Get current node memory (SAM only) -----
        if self.use_sam and self.sam_module is not None:
            # Detach memory to prevent gradient leakage
            node_memory = self.sam_module.raw_memory.detach()
        else:
            # Fallback (should not happen if SAM is enabled)
        ...
        if self.use_hct and self.hct is not None and not self.debug_simple_walk:
            ...
            # Prepare per-type embeddings for MRP
            ...
            # Project per-type embeddings if needed
        ...
        # ----- 5. Modified: Use SAM memory directly (no Base TGN embedding) -----
        # Get raw SAM memory for source/target nodes
        base_src_emb = node_memory[src_tensor]
        base_dst_emb = node_memory[dst_tensor]
        
        # Project SAM memory to hidden_dim if needed
        if self.hidden_dim != self.memory_dim:
            base_src_emb = self.simple_walk_proj(base_src_emb)
            base_dst_emb = self.simple_walk_proj(base_dst_emb)
        ...
        assert combined_walk.size(-1) == combined_base.size(-1), \
            f"Walk embedding dim {combined_walk.size(-1)} != SAM memory dim {combined_base.size(-1)}"
        ...
        # Apply fusion layer (modified for single input)
        final_src = self.fusion_layer(final_src)
        final_dst = self.fusion_layer(final_dst)
    ...
    def _prepare_per_type_embeddings(
        ...
        for wt in type_names:
            if wt in walk_data_side:
                ...
                B = src_tensor.size(0) if 'src_tensor' in locals() else node_memory.size(0)
