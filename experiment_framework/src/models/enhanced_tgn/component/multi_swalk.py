...
#  def sample_short_walks(
    ...
    #     # Expand source nodes and times to [B, W, 1] for broadcasting
    #     curr_nodes = source_nodes.unsqueeze(1).expand(-1, self.num_walks_short)  # [B, W]
    #     curr_times = current_times.unsqueeze(1).expand(-1, self.num_walks_short) # [B, W]

    #     # Initialize walk tensors
    #     walk_nodes = torch.zeros(batch_size, self.num_walks_short, self.walk_length_short,
    #                             dtype=torch.long, device=device)
    #     walk_times = torch.zeros(batch_size, self.num_walks_short, self.walk_length_short,
    #                             dtype=torch.float, device=device)
    #     walk_masks = torch.zeros_like(walk_nodes, dtype=torch.float)

    #     # First step: source nodes
    #     walk_nodes[:, :, 0] = curr_nodes
    #     walk_times[:, :, 0] = curr_times
    #     walk_masks[:, :, 0] = 1.0

    #     for step in range(1, self.walk_length_short):
    #         # Gather neighbors for all current nodes
    #         # curr_nodes: [B, W] -> indices to lookup in dense tables
    #         neighbor_ids = self.dense_neighbor_ids[curr_nodes]          # [B, W, max_deg]
    #         neighbor_times = self.dense_neighbor_times[curr_nodes]      # [B, W, max_deg]

    #         # Create temporal mask: only neighbors with time < current time
    #         time_mask = neighbor_times < curr_times.unsqueeze(-1)       # [B, W, max_deg]

    #         # Count valid neighbors per node
    #         valid_neighbor_counts = time_mask.sum(dim=-1)  # [B, W]
    #         has_valid_neighbors = valid_neighbor_counts > 0  # [B, W]
            
    #         # Compute temporal bias weights (Eq. 7)
    #         t_max = neighbor_times.max(dim=-1, keepdim=True)[0]         # [B, W, 1]
    #         # To avoid division by zero where all times are zero (padded), set t_max to at least 1
    #         t_max = torch.where(t_max > 0, t_max, torch.ones_like(t_max))
    #         weights = torch.exp((neighbor_times - t_max) / self.temperature)  # [B, W, max_deg]
    #         weights = weights.masked_fill(~time_mask, 0.0)               # zero out invalid

    #         # Convert to probabilities and sample
    #         # probs = weights / (weights.sum(dim=-1, keepdim=True) + 1e-10) # [B, W, max_deg]
            
    #         # Normalize probabilities
    #         prob_sums = weights.sum(dim=-1, keepdim=True)  # [B, W, 1]
    #         probs = weights / (prob_sums + 1e-10)  # [B, W, max_deg]
            
    #         # Handle nodes with no valid neighbors - use uniform distribution over all padded positions
    #         # Or better: keep current node (self-loop) or stop walking
    #         # Here we'll set prob to 0 for all, and handle it after sampling
            
    #         # Sample for each walk
    #         # Replace NaN probs with uniform to prevent multinomial error
    #         probs_flat = probs.view(-1, max_deg)
    #         # For rows with all zeros, set to uniform to avoid multinomial error
    #         zero_mask = probs_flat.sum(dim=-1) == 0
    #         probs_flat = probs_flat.clone()  # Avoid modifying original
    #         probs_flat[zero_mask] = 1.0 / max_deg  # Uniform distribution for empty neighbors
            
            
    #         # Sample for each walk
    #         sampled_idx = torch.multinomial(probs_flat, 1)   # [B*W, 1]
    #         sampled_idx = sampled_idx.view(batch_size, self.num_walks_short)  # [B, W]
            
    #         # if sampled_idx.max() >= max_deg or sampled_idx.min() < 0:
    #         #     logger.error(f"sampled_idx out of bounds: min={sampled_idx.min()}, max={sampled_idx.max()}, max_deg={max_deg}")
    #         #     raise RuntimeError("Sampled index out of bounds")
            
    #         if sampled_idx.max() >= max_deg:
    #             print(f"ERROR: sampled_idx.max()={sampled_idx.max()}, max_deg={max_deg}")
    #             print(f"probs has NaN: {torch.isnan(probs).any()}")
    #             print(f"probs min: {probs.min()}, max: {probs.max()}")
    #             print(f"valid_neighbor_counts min: {valid_neighbor_counts.min()}")
            
            
            
    #         # Gather the chosen neighbor IDs and times
    #         # Gather the chosen neighbor IDs and times
    #         next_node = torch.gather(neighbor_ids, -1, sampled_idx.unsqueeze(-1)).squeeze(-1)
    #         next_time = torch.gather(neighbor_times, -1, sampled_idx.unsqueeze(-1)).squeeze(-1)
            
    #         # For nodes without valid neighbors, stay at current node (self-loop) or use padding
    #         # Option 1: Stay at current node
    #         next_node = torch.where(has_valid_neighbors, next_node, curr_nodes)
    #         next_time = torch.where(has_valid_neighbors, next_time, curr_times)
    ...
    #     # Expand source nodes and times to [B, W, 1] for broadcasting
    #     curr_nodes = source_nodes.unsqueeze(1).expand(-1, self.num_walks_long)  # [B, W]
    #     curr_times = current_times.unsqueeze(1).expand(-1, self.num_walks_long) # [B, W]
        
    #     # Initialize walk tensors
    #     walk_nodes = torch.zeros(batch_size, self.num_walks_long, self.walk_length_long,
    #                             dtype=torch.long, device=device)
    #     walk_times = torch.zeros(batch_size, self.num_walks_long, self.walk_length_long,
    #                             dtype=torch.float, device=device)
    #     walk_masks = torch.zeros_like(walk_nodes, dtype=torch.float)
    ...
    #         probs = weights / (weights.sum(dim=-1, keepdim=True) + 1e-10) # [B, W, max_deg]
    #         # Sample for each walk
    #         sampled_idx = torch.multinomial(probs.view(-1, max_deg), 1)   # [B*W, 1]
    #         sampled_idx = sampled_idx.view(batch_size, self.num_walks_long) # [B, W]
