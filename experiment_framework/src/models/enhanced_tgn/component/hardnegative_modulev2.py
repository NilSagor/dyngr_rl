...
# === Dummy Hard Negative Miner for when HNM is disabled ===
class _DummyHardNegativeMiner:
    """Passthrough miner when hard negative mining is disabled."""
    def __init__(self, label_smoothing: float = 0.0):
        self.label_smoothing = label_smoothing
        self.neg_sample_ratio = 0
    
    def __call__(self, batch, memory=None, cooccurrence_matrix=None):
        # Just return batch with optional label smoothing
        if self.label_smoothing > 0 and 'labels' in batch:
            # Apply smoothing to existing labels
            labels = batch['labels'].float()
            smoothed = labels * (1 - self.label_smoothing) + 0.5 * self.label_smoothing
            batch['labels'] = smoothed
        return batch
    
    def get_cosine_scheduler(self, optimizer, num_epochs, steps_per_epoch, 
                             warmup_epochs, min_lr_ratio):
        """Return standard cosine scheduler."""
