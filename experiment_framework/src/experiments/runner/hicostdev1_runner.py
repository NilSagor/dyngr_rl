# src/experiments/runners/hicostdev1_runner.py
import torch
...
class HiCoSTdev1Runner(HiCoSTBaseRunner):
    """Runner for HiCoSTdev1 – adds explicit Co-GNN."""

    def _log_model_status(self, model: torch.nn.Module) -> None:
        logger.info(f"=== HiCoSTdev1 (Co-GNN) Model Status ===")
...
# class HiCoSTdev1Runner(BaseRunner):
#     """Runner for HiCoSTDev1 with CoGNN models."""
    ...
    #     logger.info(f"Enable co-occurrence GNN: {model.co_gnn}")
    #     logger.info(f"Walk length: {model.walk_length if model.enable_walk else 'N/A'}")
    #     logger.info(f"Num walks: {model.num_walks if model.enable_walk else 'N/A'}")
    #     logger.info("=" * 40)

    # def _profile_model(self, model: torch.nn.Module, pipeline) -> None:
    #     """Compute FLOPs for HiCoSTDev with CoGNN models."""
    #     logger.info("Computing FLOPs for HiCoSTDev with CoGNN models...")
    ...
    #         logger.info(f"HiCoST estimated FLOPs: {total_flops / 1e9:.3f} GFLOPs")
    #         logger.info(f"Parameters: {sum(p.numel() for p in model.parameters()):,}")

    #         # Optionally save trace
    #         trace_path = Path(self.config['logging']['log_dir']) / "HiCoSTDev1_trace.json"
